...
class AiqDataResource(resource.Resource):
    ...
    async def render_put(self, request) -> Message:
        try:
            ...
            print("[COAP] got request", message)
            ...
            try:
                await AiqDataManager.save_sensor_data(self.main_session, payload, self.location_id)
            except Exception as ex:
                ex.add_note("Could not store sensor data in main DB")
                await AiqDataManager.save_sensor_data(self.backup_session, payload, self.location_id)
                raise ex
